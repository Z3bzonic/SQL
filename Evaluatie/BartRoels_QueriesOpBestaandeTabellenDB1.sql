...
SELECT br.Name,City, be.Name, Alcohol FROM Brewers br
INNER JOIN Beers be
...
SELECT be.Name, Alcohol, Category, Turnover FROM Beers be
INNER JOIN Brewers br 
	ON	br.Id = be.BrewerId
INNER JOIN Categories ca
	ON ca.Id = be.CategoryId
...
SELECT Category, COUNT(be.Name) AS 'Aantal Bieren'  FROM Beers be
INNER JOIN Categories ca
	ON ca.Id = be.CategoryId
